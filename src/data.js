...
export const templates = {
  ...
  product: {
    ...
    summary: "用模板化编辑、行动入口和导出流程，把公众号内容生产做成稳定工作流。",
    html: `
      <h2>为什么企业需要标准化排版</h2>
  ...
  news: {
    ...
    html: `
      ...
      <h2>你会获得什么</h2>
      <ul><li>一套可复用的文章结构</li><li>公众号行动入口设计思路</li><li>低成本内容生产流程</li></ul>
  ...
  caseStudy: {
    ...
    html: `
      ...
      <h2>遇到的问题</h2>
      <ul><li>文章结构不稳定</li><li>读者看到中段容易流失</li><li>转化入口不统一</li></ul>
  ...
  listicle: {
    ...
    html: `
      ...
      <h2>最后一步</h2>
      <p>复制到微信后台后，再用手机预览检查行距、重点卡片和链接是否正常。</p>
    `,
  },
  newsletter: {
    category: "brand",
    ...
    html: `
      <h2>本月关键进展</h2>
      <p>完成了内容模板梳理、行动入口统一和公众号编辑流程优化。</p>
      <h2>数据观察</h2>
  ...
  course: {
    ...
    html: `
      ...
      <h2>你会获得</h2>
      <ul><li>公众号文章结构模板</li><li>行动入口设计</li><li>可复用的发布检查清单</li></ul>
    `,
  },
  recruitment: {
    category: "operation",
  ...
  brandStory: {
    ...
    html: `
      <h2>从一个真实问题开始</h2>
      <p>很多创作者写完文章后，还要花大量时间处理排版、样式和导出细节。</p>
      <h2>我们的判断</h2>
      <blockquote>好工具应该把重复劳动降到最低，把时间还给内容本身。</blockquote>
      <h2>下一步</h2>
      <p>继续补齐模板库、样式库和内容组件，让团队可以快速低成本发布内容。</p>
    `,
  },
};
...
export const sixGridItems = [
  ...
  { title: "导流", desc: "行动入口自然承接正文", color: "#7556c8", icon: "04" },
